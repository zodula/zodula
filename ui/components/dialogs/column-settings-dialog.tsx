...
export const ColumnSettingsDialog = ({
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setVisibleColumns(
        initialData?.availableColumns.map((col) => col.key) || []
      );
    } else {
      setVisibleColumns([]);
    }
  ...
  const handleApply = () => {
    const isDefault =
      ...
      JSON.stringify((initialData?.defaultColumns || []).sort());
    onClose({
      visibleColumns,
  ...
  const isAllSelected =
    visibleColumns.length === (initialData?.availableColumns.length || 0);
  const isDefault =
    JSON.stringify(visibleColumns.sort()) ===
    JSON.stringify((initialData?.defaultColumns || []).sort());
  ...
  return (
    <div className="zd:min-w-[500px] zd:flex zd:flex-col zd:gap-4">
      ...
      <div className="zd:flex zd:items-center zd:space-x-2 zd:justify-end">
        ...
        <Button onClick={handleApply} variant="solid" disabled={!hasChanges}>
          {t("Apply")}
        </Button>
      </div>
    </div>
