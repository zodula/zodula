...
const FormDebugDialog = ({
  ...
  const { originalFields, mergedFields, layoutFields, tabs, sectionsByTab } =
    initialData;
    ...
    <div className="zd:fixed zd:inset-0 zd:bg-black zd:bg-opacity-50 zd:flex zd:items-center zd:justify-center zd:z-50">
      <div className="zd:bg-white zd:rounded-lg zd:shadow-xl zd:max-w-6xl zd:max-h-[90vh] zd:w-full zd:mx-4 zd:overflow-hidden">
        <div className="zd:flex zd:items-center zd:justify-between zd:p-4 zd:border-b">
          <h2 className="zd:text-xl zd:font-semibold">
            Form Debug - Field Inspector
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="zd:p-4 zd:overflow-y-auto zd:max-h-[calc(90vh-80px)]">
          ...
          <div className="zd:mt-6">
            <h3 className="zd:text-lg zd:font-medium zd:text-red-600 zd:mb-4">
              Field Comparison
            </h3>
              ...
              <table className="zd:w-full zd:text-sm zd:border-collapse zd:border zd:border-gray-300">
                <thead>
                  <tr className="zd:bg-gray-100">
                    <th className="zd:border zd:border-gray-300 zd:p-2 zd:text-left">
                      Field Key
                    </th>
                    <th className="zd:border zd:border-gray-300 zd:p-2 zd:text-left">
                      Type
                ...
                <tbody>
                  {Object.entries(mergedFields).map(([key, field]) => {
                    const isLayoutField = layoutFields.some(
                      (lf) => lf.id === key
                    );
                    ...
                    return (
                      <tr
                        key={key}
                        className={
                          isLayoutField ? "zd:bg-green-50" : "zd:bg-blue-50"
                        }
                      >
...
export const Form = <T extends Record<string, Zodula.Field>>(
  ...
  useEffect(() => {
    ...
    if (props.values !== undefined) {
      ...
      if (shouldUpdate) {
        initialValuesRef.current = props.values
  ...
  // Doc store for fetching referenced documents
  ...
  // Enhanced onChange handler that executes scripts and handles fetch_from
  const handleChange = useCallback(
    async (fieldName: keyof T, value: any) => {
      ...
      // fetch_from format: "source_field.extend_field" or "source_field.field"
      const changedFieldName = fieldName as string;
      ...
      Object.entries(props.fields).forEach(([key, field]) => {
        if (
          field.fetch_from &&
          field.fetch_from.startsWith(changedFieldName + ".")
        ) {
          // Extract the path after the source field (e.g., "province" from "from_address.province")
          const fetchPath = field.fetch_from.substring(
            changedFieldName.length + 1
          );
      ...
      if (dependentFields.length > 0 && value) {
        const sourceField = props.fields[changedFieldName];

        // Handle Extend field type - value is already a JSON object
        if (sourceField?.type === "Extend") {
          try {
            // For Extend fields, the value is the JSON object itself
            const extendData = value;

            // Update all dependent fields by extracting nested values
            for (const dependentField of dependentFields) {
              // Support dot notation for nested fields (e.g., "extend_field.nested_value")
              ...
              if (fetchedValue !== undefined && fetchedValue !== null) {
                ...
                props.onChange?.(
                  dependentField.fieldName as keyof T,
                  fetchedValue
                );
                // Also update the form data for subsequent fetches
                updatedFormData[dependentField.fieldName] = fetchedValue;
              }
            }
          } catch (error) {
            console.warn(
              `Failed to fetch data from Extend field ${changedFieldName}:`,
              error
            );
            // Clear dependent fields if fetch fails
            for (const dependentField of dependentFields) {
              props.onChange?.(dependentField.fieldName as keyof T, null);
            }
          }
        }
        // Handle Reference field type - need to fetch the referenced document
          ...
          try {
            ...
            // For nested paths like "extend_field.nested_value", extract the root field name
            const rootFields = dependentFields
              .map((df) => {
                const pathParts = df.fetchPath.split(".");
                return pathParts[0]; // Get the root field name
              })
              .filter((field): field is string => !!field); // Filter out undefined values
            ...
            console.warn(
              `Failed to fetch data for field ${changedFieldName}:`,
              error
            );
            // Clear dependent fields if fetch fails
            for (const dependentField of dependentFields) {
