...
export default $action(async (ctx) => {
  ...
  if (!delivery_note_id) {
    return ctx.json({ delivery_note: null, trips: [], installation_notes: [], installation_percentage: 0 });
  }
  ...
  const tripIds = [...new Set((items as { parentid?: string }[]).map((i) => i.parentid).filter(Boolean))] as string[];

  const trips: Record<string, unknown>[] = [];
  for (const mid of tripIds) {
    ...
    if (m) trips.push(m as Record<string, unknown>);
  }
