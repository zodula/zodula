...
export class ZodulaDoctypeInsert<TN extends Zodula.DoctypeName = Zodula.DoctypeName> {
    ...
    private async _insert() {
        try {
            ...
            ZodulaDoctypeHelper.validateDoc(this.input, doctype.schema, this.options.bypass)
            ...
            if (!can) {
                throw new ErrorWithCode("You do not have permission to create this document", {
                    status: 403
                })
            }

            if (!userPermissionCan) {
                throw new ErrorWithCode(`You do not have User Permission to create ${this.doctypeName} document`, {
                    status: 403
                })
            }
            ...
            throw new ErrorWithCode(ZodulaDoctypeHelper.formatSqlError(error?.message), {
                status: 500,
            })
        }
    }

    private async prepareDocumentData(user: any, doctype: any): Promise<Zodula.SelectDoctype<TN>> {
        let prepared = {
            ...this.input,
    ...
    private async applyFileInsert(prepared: Zodula.SelectDoctype<TN>, doctypeName: Zodula.DoctypeName, doctype: Zodula.DoctypeSchema) {
        for (let [key, value] of Object.entries(prepared)) {
            const fieldConfig = doctype.fields[key as keyof Zodula.DoctypeSchema] as any
            ...
            if (fieldConfig?.type === "File" && value instanceof File) {
                ...
                await fs.mkdir(path.join(process.cwd(), ".zodula_data", "files", "doctypes", doctypeName, docId, fieldName), { recursive: true })
                const fileDir = path.join(process.cwd(), ".zodula_data", "files", doctypeName, docId, fieldName, filename)
                await Bun.write(fileDir, value)

                // Clean up any existing files in the directory (for consistency with update behavior)
                const files = await fs.readdir(path.join(process.cwd(), ".zodula_data", "files", doctypeName, docId, fieldName))
                for (const file of files) {
                    if (file !== filename) {
                        await fs.unlink(path.join(process.cwd(), ".zodula_data", "files", doctypeName, docId, fieldName, file))
                    }
                }
    ...
    private async executeBeforeTriggers(prepared: Zodula.SelectDoctype<TN>) {
        await loader.from("doctype").trigger(this.doctypeName, "before_insert", { old: undefined as any, doc: prepared, input: this.input })
    }

    private async executeAfterTriggers(result: Zodula.SelectDoctype<TN>) {
        await loader.from("doctype").trigger(this.doctypeName, "after_insert", { old: undefined as any, doc: result, input: this.input })
    }

    private extractRelationshipData(doctype: any, prepared: Zodula.SelectDoctype<TN>): RelationshipData {
        ...
        for (const [key, field] of Object.entries(doctype.schema.fields)) {
            ...
            if (fieldConfig.type === "Reference Table" && fieldConfig.reference) {
                refTableList[key] = prepared[key as keyof Zodula.SelectDoctype<TN>] as any[]
                delete prepared[key as keyof Zodula.SelectDoctype<TN>]
            }
        }
    ...
    private async insertMainDocument(db: BunQL, doctype: any, prepared: Zodula.SelectDoctype<TN>) {
        ...
        const preparedFields = Object.keys(prepared).map((field: any) => `"${field}"`)
        const preparedValues = Object.values(prepared).map((value: any) => value !== null && value !== undefined ? `${value}` : null)
        const query = `INSERT INTO "${doctype?.name}" (${preparedFields}) VALUES (${preparedValues.map(v => "?").join(", ")})`
        await db.run(query, preparedValues)
        const returned = (await db.get(`SELECT ${returnFields} FROM "${doctype?.name}" WHERE id = '${prepared.id}'`)) as any
        return returned
    }
    ...
    private async insertRelationships(
        ...
        await this.insertExtendRelationships(db, doctype, result, relationshipData.extendsList)
        await this.insertRefTableRelationships(db, doctype, result, relationshipData.refTableList)
    }
    ...
    private async insertExtendRelationships(
        ...
        for (const [key, payload] of Object.entries(extendsList)) {
            ...
            const refDoctypeSchema = loader.from("doctype").get(fieldConfig?.reference as Zodula.DoctypeName)?.schema
            ...
            try {
                ...
                ZodulaDoctypeHelper.formatDoc(formattedPayload as Zodula.SelectDoctype<TN>, refDoctypeSchema)
                const insertedPayload = await zodula.doctype(refDoctypeName).insert(formattedPayload).bypass(this.options.bypass)
                    ; (result as any)[key] = insertedPayload
            } catch (error) {
                throw new ErrorWithCode(`Error inserting ${this.doctypeName}/${result.id}/${key}: ${error}`, {
                    status: 500,
                })
            }
        }
    ...
    private async processVectorFields(result: Zodula.SelectDoctype<TN>, doctype: Zodula.DoctypeSchema) {
        try {
            // Detect Vector fields in the document
            const vectorFields = ZodulaDoctypeHelper.detectVectorFields(doctype, result)

            if (vectorFields.length > 0) {
                // Format vector data for embeddings processing
                const vectorData = ZodulaDoctypeHelper.formatVectorData(result.id!, vectorFields)

                console.log(`TODO: Process Vector fields for ${this.doctypeName}/${result.id}:`)
            ...
            console.error(`Failed to process Vector fields for ${this.doctypeName}/${result.id}:`, error)
