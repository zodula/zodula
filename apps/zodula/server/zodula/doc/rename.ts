...
export class ZodulaDoctypeRename<TN extends Zodula.DoctypeName = Zodula.DoctypeName> {
    ...
    constructor(doctypeName: TN, private oldId: string, private newId: string) {
    ...
    private async _rename() {
        ...
        const old = await zodula.doctype(this.doctypeName).get(this.oldId).bypass(true).unsafe()
        ...
        if (!can) {
            throw new ErrorWithCode(`You do not have User Permission to rename ${this.doctypeName} document with id ${this.oldId}`, {
                status: 403
            })
        }
    ...
    private validateDocument(old: Zodula.SelectDoctype<TN> | null) {
        if (!old) {
            throw new Error(`Document with id ${this.oldId} not found`, { cause: 404 })
        }
        if (old.doc_status === 2) {
            throw new ErrorWithCode(`Cannot rename cancelled document. Document with id ${this.oldId} has been cancelled.`, { status: 400 })
        }
    }
    ...
    private async updateMainDocument(db: BunQL, doctype: any, old: Zodula.SelectDoctype<TN>) {
        ...
        const returned = (await db.get(`SELECT ${returnFields} FROM "${doctype?.name}" WHERE id = '${this.newId}'`)) as any
        return returned
    }
    ...
    private async executeBeforeTriggers(
        ...
        const isSaveAfterSubmit = old?.doc_status === 1 && prepared.doc_status === 1
        const isDraft = old?.doc_status === 0

        if (isSaveAfterSubmit) {
            await loader.from("doctype").trigger(this.doctypeName, "before_save_after_submit", { old, doc: prepared, input: undefined })
        }

        if (isDraft) {
            await loader.from("doctype").trigger(this.doctypeName, "before_save", { old, doc: prepared, input: undefined })
        }

        await loader.from("doctype").trigger(this.doctypeName, "before_change", { old, doc: prepared, input: undefined })
    }
    ...
    private async executeAfterTriggers(
        ...
        if (isSaveAfterSubmit) {
            await loader.from("doctype").trigger(this.doctypeName, "after_save_after_submit", { old, doc: result, input: undefined })
        }

        if (isDraft) {
            await loader.from("doctype").trigger(this.doctypeName, "after_save", { old, doc: result, input: undefined })
        }

        await loader.from("doctype").trigger(this.doctypeName, "after_change", { old, doc: result, input: undefined })
    }

    private async updateReferenceFields(db: BunQL, doctype: any, oldId: string, newId: string) {
        ...
        for (const doctype of allDoctypes) {
            ...
            for (const [fieldName, fieldConfig] of Object.entries(doctypeFields)) {
                ...
                if ((field.type === "Reference") && field.reference === this.doctypeName) {
                    if (fieldName) {
                        const existings = await zodula.doctype(doctype.name).select().where(fieldName as any, "=", oldId).bypass(true)
    ...
    private async updateAuditTrailReferences(db: BunQL, oldId: string, newId: string) {
