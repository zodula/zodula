...
export default $action(
  async (ctx) => {
      ...
      try {
        ...
        if (letterHead) {
          if (letterHead.content) {
            letterHeadTemplate = letterHead.content;
          }
          if (letterHead.css_content) {
            letterHeadCss = letterHead.css_content;
          }
        }
      } catch (error) {
    ...
    try {
      ...
      console.warn(
        "Could not fetch default currency from Currency doctype:",
        error
      );
    }
    ...
    // Combine letter head CSS with template CSS
    ...
    if (template && template.is_custom && template.html) {
      ...
      templateContent = convertLayoutToTemplate(
        template.layout,
        doctypeFields,
        combinedCss,
        db,
    ...
    // Helper function to render letter head with async support using binba
    const renderLetterHeadAsync = async (
      template: string,
      context: any
    ): Promise<string> => {
      if (!template) return "";
      ...
      try {
        // Binba supports native async rendering with Jinja2-like syntax
        // It already supports {{ }} for variables and {% %} for control structures
        // No preprocessing needed - binba handles async/await natively
        return await Template.render(template, context, {
          autoescape: false,
        });
      } catch (error) {
        console.warn("Error rendering letter head template with binba:", error);
        return "";
      }
    };
    ...
    for (const doc of documents) {
      try {
        ...
        // Render letter head content with async support
        const letterHeadContext = {
          doc,
          user,
          lang: lang || "en",
          zodula,
        };
        letterHeadContent = await renderLetterHeadAsync(
          letterHeadTemplate,
          letterHeadContext
        );
...
function convertLayoutToTemplate(
  ...
  try {
    ...
    return generateTemplateRecursive(
      layout,
      doctypeFields,
      customCSS,
      db,
...
function generateTemplateRecursive(
  layout: any[],
  doctypeFields: Record<string, Zodula.Field> = {},
  customCSS: string = "",
  db?: any,
    ...
    try {
      ...
      if (specifiedFields.length > 0) {
        ...
        displayFields = specifiedFields
          ...
          .filter(
            ([fieldName, fieldConfig]: [string, Zodula.Field]) => fieldConfig
      ...
      if (displayFields.length > 0) {
        ...
        tableHtml += `            {% else %}\n`;
        tableHtml += `                <div class="table-row">\n`;
        tableHtml += `                    <div class="table-cell" colspan="${displayFields.length}"></div>\n`;
        tableHtml += `                </div>\n`;
        tableHtml += `            {% endif %}\n`;
      } else {
...
function renderExtendFieldRecursive(
  ...
  if (fieldConfig.reference) {
    try {
      ...
      const defaultLayout = generateDefaultLayoutForDoctype(
        referenceDoctype,
        referenceFields
      );
      ...
      extendHtml += renderLayoutRecursive(
        defaultLayout,
        referenceFields,
        nestedDocPrefix,
        defaultCurrency,
...
function generateDefaultLayoutForDoctype(
  doctypeMetadata: DoctypeMetadata,
  doctypeFields: Record<string, Zodula.Field>
): any[] {
  const layout: any[] = [];
      ...
      for (const field of item) {
        if (
          ...
          if (fieldType === "Reference Table" && fieldConfig) {
            const tableHtml = renderReferenceTableField(
          ...
          if (fieldType === "Extend" && fieldConfig) {
            const extendHtml = renderExtendFieldRecursive(
              field.value,
              fieldConfig,
              doctypeFields,
              defaultCurrency,
