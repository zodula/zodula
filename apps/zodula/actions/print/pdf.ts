...
function getDoctypeMetadata(doctypeName: string): {
  ...
  try {
    ...
    return {
      ...
      isSingle: !!(metadata.schema.is_single === 1 || metadata.schema.is_single),
    };
    } catch (error) {
      console.warn(
...
export default $action(
  async (ctx) => {
    ...
    const { metadata: doctypeMetadata, fields: doctypeFields, label: doctypeLabel, isSingle } =
...
function renderReferenceTableField(
  ...
  try {
    ...
    const displayFields = specifiedFields.length > 0
      ? specifiedFields
          .map((name: string) => [name, refFields[name]] as [string, Zodula.Field])
          .filter(([, config]: [string, Zodula.Field]) => config)
      : Object.entries(refFields)
          .filter(([name, config]: [string, Zodula.Field]) => 
            !config.no_print &&
...
function renderExtendFieldRecursive(
  ...
  if (!fieldConfig.reference) {
    ...
    const defaultLayout = generateDefaultLayout(referenceDoctype, referenceFields);
...
function generateDefaultLayout(
  ...
  if (tabs && tabs.length > 0) {
    ...
    layout.push(...createFieldRows(printableFields, fieldsInLayout, sectionLabel));
  }
    ...
    if (Array.isArray(item)) {
      ...
      for (const field of item) {
        if (typeof field === "object" && field !== null && field.type === "field") {
          html += renderFieldInRow(field, doctypeFields, docPrefix, defaultCurrency, parentField);
        }
      }
      html += `    </div>\n`;
      ...
      if (item.type === "section") {
        ...
        html += renderField(item, doctypeFields, docPrefix, defaultCurrency, parentField);
      }
    }
  }
...
function renderFieldInRow(
  ...
  return renderStandardField(field, fieldConfig, alignClass, docPrefix, defaultCurrency);
}
