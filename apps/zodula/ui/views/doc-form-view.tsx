...
import { Save, Copy, ArrowLeft, ArrowRight, Printer, MoreHorizontal, Trash2, SaveIcon, RotateCcw } from "lucide-react";
...
import { KeyboardShortcuts, KeyboardShortcutsDialog } from "../components/custom/keyboard-shortcuts";
...
export function DocFormView({
    ...
    // ===== RELATIVES COUNT =====
    const { data: relativeCount, loading: relativeCountLoading, error: relativeCountError, reload: reloadRelativeCount } = useAction("zodula.core.count", {
        data: {
            docFilters: relatives.reduce((acc, relative) => {
    ...
    React.useEffect(() => {
        if (mode === 'create' && fields && doctypeDoc) {
            ...
            fields.forEach((field) => {
                if (field.doctype === doctype && field.default !== undefined && field.default !== null && field.name !== "doc_status") {
    ...
    const handlePrint = async () => {
        ...
        try {
            const result = await popup(PrintTemplateDialog, {
    ...
    const handleCancel = async () => {
        try {
            const con = await confirm({
                message: "Are you sure you want to cancel this document ? This action cannot be undone. The changes you made will be lost.",
                variant: "destructive"
            });
            if (con) {
    ...
    const handleCreate = async () => {
        setIsLoading(true);
        try {
            const createdDoc = await zodula.doc.create_doc(doctype as Zodula.DoctypeName, formData);
            if (createdDoc) {
                if (cbUrl) {
                    let obj = fromDoc || {};
                    if (fromField) {
        ...
        <div className="zd:space-y-6">
            {mode === 'create' ? (
                ...
                <div className="zd:space-y-3">
                    <h3 className="zd:text-sm zd:font-medium zd:text-muted-foreground">New Document</h3>
                ...
                /* Activity Log */
                <div className="zd:space-y-3">
                    <h3 className="zd:text-sm zd:font-medium zd:text-muted-foreground">Relatives</h3>
                    <div className="zd:flex zd:flex-col zd:gap-2">
                        {relatives.map((relative) => {
                            const filterQuery = encodeURIComponent(`[["${relative.child_field_name}", "=", "${id}"]]`);
                            return (
                                <Link
                                    ...
                                    key={relative.id}
                                >
                                    {relative.child_doctype} ({relativeCount?.results?.find((result: any) => result.doctype === relative.child_doctype)?.count || 0})
                                </Link>
                            )
                        })}
                    </div>
    ...
    // ===== KEYBOARD SHORTCUTS =====
    const shortcuts = [
        { keys: `${modifierKey} + S`, description: mode === 'create' ? 'Create' : 'Save' },
    ...
    useCmd(KEYBOARD_SHORTCUTS.SAVE, mode === 'create' ? handleCreate : handleSave, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.UPDATE, handleUpdate, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.SUBMIT, handleSubmit, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.CANCEL, handleCancel, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.DELETE, handleDelete, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.DUPLICATE, handleDuplicate, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.RELOAD, handleReload, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.PRINT, handlePrint, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.NEW, () => push(`/desk/doctypes/${doctype}/form`, { state: { resetForm: true } }), { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.LIST, () => push(`/desk/doctypes/${doctype}/list`), { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.BACK, () => push(`/desk/doctypes/${doctype}/list`), { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.RESET, reset, { disabled: isSystemGenerated });
    useCmd(KEYBOARD_SHORTCUTS.HELP, async () => {
    ...
    return (
        <NavbarLayout>
            <SidebarLayout
                title={<div className="zd:flex zd:gap-2 zd:items-center">
                    {mode === 'create' ? "New Document" : (isSingle ? doctypeLabel : (doc?.id || "New Doc"))}
                    <span className="zd:flex zd:gap-2 zd:items-center no-print">
                        {doctypeDoc?.is_submittable === 1 && <DocStatusBadge status={doc?.doc_status || 0} />}
                    </span>
                </div>}
                subtitle={isSingle ? "" : t(doctypeLabel)}
                sidebarContent={sidebarContent}
                actionSection={doctypeDoc?.is_system_generated === 1 ? <div>
                    <span className="zd:text-sm zd:text-muted-foreground">This is a system generated doctype. You cannot {mode === 'create' ? 'create' : 'edit'} this doctype.</span>
                </div> : <div className="zd:flex zd:items-center zd:gap-2">
                    {mode === 'edit' && (
                        <>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={handleDuplicate}>
                                        ...
                                        <kbd className="zd:ml-2 zd:px-1 zd:py-0.5 zd:text-xs zd:bg-muted zd:rounded">Ctrl + D</kbd>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={handleReload}>
                                        <RotateCcw className="zd:w-4 zd:h-4 zd:mr-1" />
                                        <span className="zd:flex-1">{t("Reload")}</span>
                                        <kbd className="zd:ml-2 zd:px-1 zd:py-0.5 zd:text-xs zd:bg-muted zd:rounded">Ctrl + R</kbd>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={handleCancel} className={cn(
                                        "zd:text-red-600 zd:focus:text-red-600",
                                        doc?.doc_status === 1 ? "" : "zd:hidden"
                                    )}>
                                        <Copy className="zd:w-4 zd:h-4 zd:mr-1" />
                                        <span className="zd:flex-1">{t("Cancel")}</span>
                                        <kbd className="zd:ml-2 zd:px-1 zd:py-0.5 zd:text-xs zd:bg-muted zd:rounded">Ctrl + Shift + C</kbd>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={handleDelete} className={cn(
                                        "zd:text-red-600 zd:focus:text-red-600",
                                        doc?.doc_status !== 1 ? "" : "zd:hidden"
                                    )}>
                                        <Trash2 className="zd:w-4 zd:h-4 zd:mr-1" />
                                        <span className="zd:flex-1">{t("Delete")}</span>
                                        <kbd className="zd:ml-2 zd:px-1 zd:py-0.5 zd:text-xs zd:bg-muted zd:rounded">Ctrl + Delete</kbd>
                                    </DropdownMenuItem>
                                </DropdownMenuContent>
                            </DropdownMenu>
                        </>
                    ...
                    <div className="">
                        ...
                        <div className="zd:flex zd:gap-2 zd:items-center zd:mt-2">
                            <span className="zd:text-sm zd:text-muted-foreground">
                                {doc?.created_by ? (getUserName(doc.created_by) || doc.created_by) : "Unknown"} {t("Created At")} {zodula.utils.formatTimeAgo(doc?.created_at || "")}
                            </span>
                            {/* center dot */}
                            <span className="zd:text-sm zd:text-muted-foreground">•</span>
                            <span className="zd:text-sm zd:text-muted-foreground">
                                {doc?.updated_by ? (getUserName(doc.updated_by) || doc.updated_by) : "Unknown"} {t("Updated At")} {zodula.utils.formatTimeAgo(doc?.updated_at || "")}
                            </span>
                        </div>
                    </div>
                </div>
