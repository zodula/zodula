...
export function useListParams(): ListParams & ListParamsActions {
    ...
    const updateSearchParams = useCallback((updates: Partial<ListParams>) => {
        const newSearch = { ...search, ...updates };
        // Handle filters specially - encode as JSON
        if (updates.filters) {
            newSearch.filters = encodeURIComponent(JSON.stringify(updates.filters)) as any;
        }
    ...
    const handleLimitChange = useCallback((newLimit: number | null) => {
        if (newLimit == 20) {
    ...
    const handleSort = useCallback((field: string) => {
        ...
        updateSearchParams({
            ...(newSort ? { sort: newSort } : { sort: null }),
            ...(newOrder ? { order: newOrder } : { order: null }),
        });
    ...
    const handleOrderChange = useCallback((newOrder: "asc" | "desc" | null) => {
    ...
    const handleApplyFilters = useCallback((newFilters: IFilter<any, any, IOperator>[] | null) => {
