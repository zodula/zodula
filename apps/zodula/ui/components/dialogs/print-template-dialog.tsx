...
interface PrintTemplateDialogProps {
    isOpen: boolean;
    onClose: (result?: { templateId: string; templateName: string } | null) => void;
    initialData?: {
...
export function PrintTemplateDialog({ isOpen, onClose, initialData }: PrintTemplateDialogProps) {
    ...
    useEffect(() => {
        if (letterHeads?.length > 0) {
            setSelectedLetterHead(letterHeads[0]?.id || null);
        }
    }, [letterHeads]);
    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            if (printWindow) {
                ...
                onClose({ templateId: selectedTemplate || "", templateName: template?.name || "" });
            }
        } catch (error) {
