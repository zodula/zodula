...
export const Navbar = ({ children }: NavbarProps) => {
    ...
    const handleAboutZodula = async () => {
        await popup(AboutZodulaDialog, {
            ...
            description: "Modern, flexible framework for building web applications with a focus on developer experience and scalability.",
    ...
    return (
        <>
            <div className="zd:flex zd:w-full zd:items-center zd:justify-center zd:border-b no-print">
                <div className={cn("zd:flex zd:gap-4 zd:items-center zd:justify-between zd:px-4 zd:py-2 zd:w-full zd:max-w-8xl", fullWidth ? "zd:max-w-screen" : "")}>
                    <div className="zd:relative zd:group zd:flex zd:items-center zd:gap-2 zd:flex-1">
                        <Link to="/desk" className={cn(
                            "zd:text-xl zd:font-bold zd:flex zd:items-center zd:gap-2  zd:w-10 zd:h-10",
                        )}>
                            <img src={!!zodula__WebsiteSetting?.logo ? zodula.utils.getDoctypeFileUrl("zodula__Global Setting", zodula__WebsiteSetting?.id || "", "logo", zodula__WebsiteSetting?.logo as string || "") + "?w=40&h=40" : "/public/zodula/zodula-logo.png"} alt="Logo" className={cn(
                                "zd:rounded"
                            )} />
                        </Link>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuLabel className="zd:text-sm zd:flex zd:flex-col">
                                        ...
                                        <span className="zd:text-xs zd:text-muted-foreground zd:truncate zd:max-w-40"> {user?.id} </span>
                                    </DropdownMenuLabel>
                                    ...
                                    <DropdownMenuItem onClick={handleLogout} className="zd:text-red-600 zd:focus:text-red-600">
                                        {t("Logout")}
                                    </DropdownMenuItem>
                                </DropdownMenuContent>
                            </DropdownMenu>
