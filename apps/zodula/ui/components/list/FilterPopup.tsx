...
export function FilterPopup({
    ...
    const getFieldPlugin = (fieldName: string) => {
        ...
        return plugins.find(plugin =>
            plugin.types.includes(field.type as any)
        );
    };
    ...
    const addFilterRow = () => {
        ...
        setFilterRows([...filterRows, { id: newId, field: "", operator: "=", value: "" }]);
    };
    ...
    const handleApplyFilters = () => {
        const validFilters: IFilter<any, any, IOperator>[] = filterRows
            .filter(row => row.field && row.operator && (row.value !== undefined || ["is null", "is not null"].includes(row.operator)))
            .map(row => {
                ...
                if (["IN", "NOT IN"].includes(row.operator)) {
                    ...
                    const numValue = floatRegex.test(row.value) ? parseFloat(row.value) : intRegex.test(row.value) ? parseInt(row.value) : row.value;
                    value = numValue
                }
    ...
    useEffect(() => {
        setFilterRows(filters?.map((filter, index) => ({
            id: `${index + 1}`,
            field: filter[0] as string,
            operator: filter[1],
            value: String(filter[2]),
    ...
    return (
        <Popover open={open} onOpenChange={onOpenChange}>
            ...
            <PopoverContent className="zd:w-96 zd:bg-background zd:p-2 zd:border zd:rounded zd:min-w-[480px]" align="end">
                        ...
                        {filterRows.map((row) => (
                            <div key={row.id} className="zd:flex zd:items-center zd:gap-2">
                                <Select
                                    ...
                                    value={row.field}
                                    onChange={(value) => updateFilterRow(row.id, { field: value })}
                                    className="zd:w-40"
                                />

                                <Select
                                    displayMode="label"
                                    options={OPERATORS.map(operator => ({ value: operator.value, label: operator.value, subtitle: operator.label }))}
                                    value={row.operator}
                                    onChange={(value) => updateFilterRow(row.id, { operator: value as IOperator })}
                                    className={cn(
                                        ["IS NULL", "IS NOT NULL"].includes(row.operator) ? "zd:w-full" : "zd:w-24"
                                    )}
                                />
