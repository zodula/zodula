...
import type { ListParams, ListParamsActions } from "../../hooks/use-list-params";
...
export function ListView({
    ...
    const listViewFields = useMemo(() => {
        ...
        return fields.filter((field: any) =>
            field.doctype === doctype && field.in_list_view
    ...
    const allAvailableColumns: ListColumn[] = useMemo(() => {
        ...
        const displayFieldInfo = fields.find((field: any) => field.name === displayField);
        const displayPlugin = displayFieldInfo ? plugins.find(plugin => plugin.types.includes(displayFieldInfo.type)) : null;

        cols.push({
            key: displayField,
            label: displayField === "id" ? "ID" : displayField,
            sortable: true,
            render: displayPlugin && displayFieldInfo ? (doc: any) => displayPlugin.cellRender({
        ...
        fields.forEach((field: any) => {
            ...
            if (field.name !== displayField) {
                ...
                const plugin = plugins.find(plugin => plugin.types.includes(field.type));

                cols.push({
                    key: field.name,
                    label: field.label || field.name,
                    sortable: field.type !== "Reference Table" && field.type !== "Extend",
                    render: plugin ? (doc: any) => plugin.cellRender({
                        fieldOptions: field,
    ...
    const derivedColumns: ListColumn[] = useMemo(() => {
        return allAvailableColumns.filter(col => visibleColumns.includes(col.key)).map(col => ({
            ...col,
    ...
    const handleColumnSettings = async () => {
        const result = await popup(ColumnSettingsDialog, {
    ...
    return (
        <div className="zd:flex zd:flex-col zd:gap-4 zd:pb-12">
            <ListToolbar
                searchPlaceholder={t(`Search By`) + " " + t(`${!doctypeDoc?.search_fields ? "ID" : doctypeDoc?.search_fields?.split("\n").join(", ")}`)}
