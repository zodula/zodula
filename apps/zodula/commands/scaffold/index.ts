...
async function scaffoldApp() {
    ...
    await Bun.write(`${appDir}/package.json`, JSON.stringify(packageJson, null, 2));
    ...
    await Bun.write(`${appDir}/ui/pages/shell.tsx`, `interface ShellProps {
...
async function scaffoldDoctype() {
    ...
    const { appName, doctypeName, domain, label, isSubmittable } = await inquirer.prompt([
        {
            type: "list",
            name: "appName",
            message: "Select app:",
    ...
    const { actionPath } = await inquirer.prompt([
        {
            ...
            validate: (input: string) => {
                ...
                if (!/^[a-z][a-z0-9_-]*\.[a-z][a-z0-9_-]*\.[a-z][a-z0-9_]*$/.test(input)) {
...
async function scaffoldBackground() {
    const { backgroundPath } = await inquirer.prompt([
        {
            type: "input",
            name: "backgroundPath",
            message: "Background job path (e.g., zodula.core.email, zerp.report.sales):",
            validate: (input: string) => {
    ...
    logger.info(`Creating background job: ${backgroundName} in ${appName}/${module}`);
